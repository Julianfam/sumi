...
export function Menu() {
  ...
  return (
    <div className="screen-enter mx-auto grid w-full max-w-lg flex-1 content-center gap-10 self-stretch lg:max-w-none lg:grid-cols-[minmax(0,26rem)_minmax(0,1fr)] lg:items-center lg:gap-16">
      <div className="stagger-in flex flex-col gap-8 sm:gap-10">
        <header className="flex flex-col items-start gap-3">
          <p className="text-2xs font-medium tracking-label text-muted uppercase">
            Sudoku de tinta
          </p>
          <h1 className="font-display text-5xl font-medium text-ink sm:text-6xl">
            Sumi
          </h1>
            ...
            return (
              <li key={d}>
                <button
                  type="button"
                  onClick={() => startNew(d)}
                  className="group flex w-full items-center gap-4 rounded-xl bg-surface px-4 py-3.5 text-left paper-shadow transition-[transform,background-color] duration-150 ease-[var(--ease-smooth-out)] hover:-translate-y-px hover:bg-selected active:scale-96"
                >
                  <span className="font-display w-8 text-lg text-primary tabular-nums">
                    {meta.rank}
                  </span>
